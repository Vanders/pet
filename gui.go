	...
	for {
		...
		case *sdl.KeyboardEvent:
			...
			if nextEvent != nil {
				...
				g.lastChar = rune(inputEvent.Text[0])
			}

			// Lookup g.lastChar and replace the scancode if we have a match
			if g.lastChar != rune(0) {
				scancode, ok := charscans[g.lastChar]
				if ok {
					k.Scancode = scancode
				}
			}
